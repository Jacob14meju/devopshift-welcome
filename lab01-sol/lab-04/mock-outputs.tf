...
locals {
  ...
  services = var.create-database ? ["web", "api", "database"] : ["web", "api"]
}
...
output "database" {
  value = var.create-database ?  "A mock database will be created for this environment." : "No database needed for this environment."
  description = "is there any database required"
}

output "mock-database" {
  value = [for x in local.local.services : "configured ${x} service"]
}
